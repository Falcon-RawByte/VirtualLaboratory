	...
	render(){
		...
		return (
			<div id="sessionBlock">
				...
				Наборы параметров эксперимента
				</div>
				<div id="sessionListBlock" style={{display:'flex',flexDirection:'row'}}>
					<div style={{borderRight:'2px solid black'}}>
						<div className="sessionCreationBlock" style={{display:'flex',flexDirection:'column'}}>
							<div className="noSelect" style={{textAlign:'center'}}>Название набора</div>
							<input type="text" value={this.state.sessionName} onChange={this._changeName}/>
							<button className="btn btn-primary presetButton" onClick={this._saveSession}>Добавить набор параметров</button>
							<button className="btn btn-primary presetButton" onClick={this._newSession}>Создать новый набор</button>
						</div>
						<div className="sessionCreationBlock" style={{display:'flex',flexDirection:'column',borderTop:'2px solid black'}}>
								<label className="btn btn-primary presetButton" htmlFor="fileField">Загрузить из файла</label>
								<input id="fileField" type="file" onChange={this._loadFile} style={{display:'none'}}/>
								<button className="btn btn-primary presetButton" onClick={this._saveFile}>Сохранить в файл</button>
						</div>
					</div>
					{list}
				</div>
